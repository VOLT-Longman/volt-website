...
## J-2 시네마틱 히어로 정비 (2026-07-18)

- J-1의 5장 자동 슬라이드·크로스페이드·Ken Burns 배경을 물류 현장을 담은 대표 장면 1장으로 교체했다. 메시지보다 이미지 전환이 앞서 보이던 문제를 없애고, 한 번의 방문에서 같은 브랜드 인상을 유지한다.
- 히어로 스크롤 패럴랙스와 CTA 마그네틱 이동을 제거했다. 입장 모션만 짧게 유지해 콘텐츠가 움직이지 않고, 클릭 위치도 안정적이다.
- 데스크톱과 모바일의 첫 화면 안에 핵심 문구·CTA·지표가 들어오도록 여백과 타이포 규모를 재조정했다. 모바일은 부연 설명을 숨기고 CTA를 2열+주 행동 1열로 배치한다.
- 대표 이미지의 데스크톱/모바일 원본만 유지하고 나머지 8개 슬라이드 에셋을 제거했다. 시각 회귀 테스트도 실제 대표 이미지가 로드된 뒤 촬영하도록 바꿨다.

## J-1 시네마틱 히어로 배경 (2026-07-18)

- 홈 히어로 배경을 스타필드 캔버스에서 함대 이미지(RSI 공식 Hercules Starlifter 렌더 5장,
  © Cloud Imperium Games) 크로스페이드 + 켄 번즈 시네마틱으로 교체했다. 히어로 구조·타이포는
  그대로다. 첫 장만 프리로드하고 나머지는 유휴 시점에 로드하며, 터치/reduced-motion 환경은
  켄 번즈 없이 정적·크로스페이드만 유지한다(H-1 성능 원칙). 푸터에 팬 콘텐츠 면책 문구를 추가했다.

## I-2 Liquid Glass 제거 — 플랫 다크 복원 (2026-07-18)

- 라이브 적용 후 PM 판정으로 I-1 계열 글래스 표면 처리(글래스 토큰·카드 글래스 톤·블러 크롬
  오버라이드·포인터 추적 스펙큘러 하이라이트)를 전면 제거하고, 각 표면을 I-1 이전
  v1.0-quality 릴리스 기준 디자인으로 복원했다. 콘센트릭 라운딩 토큰과 I-1.7~I-1.8의
  radius 정합 수정은 글래스와 무관한 위생 개선이라 유지한다.
- 추천무역품 카드의 "이 상품 선택" 버튼이 카드마다 크기가 달라지던 문제를 flex column +
  하단 고정으로 수정했다.
