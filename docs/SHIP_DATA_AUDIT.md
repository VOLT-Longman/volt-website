...
- `plannerEligible: boolean` — VOLT 무역플래너 노출 override. `false`는 강제 제외, `true`는 우선 노출, 미지정은 구현됨 + cargo > 0이면 기본 노출
...
무역플래너는 순수 화물선만 고르는 목록이 아니라, 사용자가 보유한 cargo 보유 함선으로 무역 가능성을 계산하는 도구입니다.
...
- `implemented: true`
- cargo 값이 0보다 큼
- 화물, 물류, 수송, 무역, 운송, 보급, 산업 등 무역플래너에서 우선 노출할 가치가 큼
...
- 구현됨 + cargo > 0이면 무역플래너에 기본 노출합니다.
- 전투, 탐사, 다목적 태그가 있어도 cargo가 있으면 검색 가능해야 합니다.
- 정렬 점수에서 화물/물류/수송/무역 계열을 우선 배치합니다.
