...
원칙(기존과 동일): 대규모 리팩터 지양, 독립 커밋 단위, 게이트 통과 후 반영. 아래에는 완료된 작업의 원래 계획과 완료 기록을 함께 보존한다.

## 완료 현황 (2026-07-13)

| 항목 | 결과 |
|---|---|
| H-1 | `pointer: fine` 환경에서만 스타필드 RAF·시차 효과를 유지하고, 터치·reduced-motion 환경은 정적 1회 렌더로 전환. 터치/마우스 렌더 횟수 스모크 추가. |
| H-2 | 320/390/430/768px의 주요 섹션 overflow 가드와 844×390 가로모드 홈 검증 추가. |
| H-3 | 연혁·무역허브 피처·가입 단계·정책·FAQ·무역가이드를 `js/site-content.js`로 분리. `main.js`는 1,629줄로 감소했고 호출부는 shim으로 유지. |
| H-4 | 미참조 `assets/images/VOLT_logo.png`(233KB) 삭제. |
| H-5 | 웹폰트 요청을 추가하지 않고 `Pretendard` 항목을 공개 사이트·관리자 CMS 스택에서 제거. 관리자 CSS는 함께 포맷해 유지보수성을 높임. |

정정: Pretendard는 번들·CDN으로 제공되지 않았지만, 사용자 기기에 로컬 설치된 경우에는 매치될 수 있었다. 이번 선택은 네트워크 비용을 늘리지 않고 실제 제공 정책과 스택을 일치시키는 B안이다.
