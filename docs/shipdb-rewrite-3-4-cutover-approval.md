...
## 7. role 필터 UX 제안 (실전 ON 전 확정, PM 지시)

- **문제**: 현재 ON 필터는 canonical role 52종을 가로 칩으로 노출. 사실성(계약: canonical 집합·버킷팅 없음)은 지키나 52개 가로 칩은 탐색성이 나쁘다. **52칩을 최종 공개 UI로 확정하지 않는다.**
- **제안(분류·버킷팅 없음, Erkul role 원문 유지)**: 칩 그리드 → **단일 "역할 선택" 검색형 컨트롤**.
  - 하나의 콤보박스/드롭다운(예: `<select>` 또는 검색 입력+리스트박스). 옵션 = canonical role 52종 그대로(Erkul EN 키, KO 라벨), **묶음·career 그룹핑 없음**(계약 2항 준수).
  - 타이핑 시 role 라벨(KO)·원문(EN)로 **필터링 검색** — "화물" 입력 → 경/중형/대형 화물선 등 매칭 옵션만 표시. 선택 시 해당 canonical role로 목록 필터(현 매처 재사용).
  - 단일 선택(현 다중 칩 대비 단순화). '전체'는 선택 해제.
  - 접근성: 표준 폼 컨트롤이라 키보드·스크린리더 자연 지원. 모바일에서 가로 스크롤 칩보다 우수.
- **불변식**: 역할 사실은 canonical role 그대로(추가 분류·상위 묶음 도입 없음). KO는 기존 `localization-roles.json` 번역 재사용.
- **범위/시점**: 구현은 별도 원자 커밋(ON 전용 UI, OFF 불변). **PM이 이 안을 승인하면** 실전 ON 전에 반영. 현재는 제안만(코드 미변경).

## 8. PM 승인 포인트 (다음 결정)

2.7·role 이관·CI가 끝났으므로, 다음은 **실전 전환·삭제**의 별도 판정이다.

- **A. 3.5 착수 승인** — 플래그 동시 ON(서버+클라) + 이중 게이트(파이프라인 미산출[완료]+잔존 리더 0[검증됨]) → 레거시 삭제(단일 revert 커밋). role 필터 UX(§7) 반영 포함.
- **B. role UX 먼저** — §7 단일 검색형 role 필터를 먼저 원자 구현(ON 전용, OFF 불변)한 뒤, 그 결과를 보고 3.5 판정.
- **C. 보류** — 특정 잔여 리스크를 먼저 확정한 뒤 3.5.

*(이전 §6-7의 A/B/C 승인 포인트는 PM B 판정으로 갱신됨 — 위가 현행 결정 지점.)*
