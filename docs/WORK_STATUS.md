...
**ShipDB Erkul 재작성 v2 전 단계 완료 · 위생 패치 종료 · 공지/갤러리 날짜 계약 정리 완료.**

**ShipDB**
- 공개 249척(Erkul canonical 219 + RSI 공식 30)이 **유일 사실원**. 이중 읽기 플래그는
  서버·클라이언트 양쪽에서 **완전 제거**(`2ec4dee`) — 되돌림 경로는 코드에 없다.
- 삭제 완료: `data/volt-data.js`의 ships 배열, `data/ship-en.js`, `data/ship-prices-usd.json`,
  레거시 재생성 스크립트 5종. 계약 테스트가 **부재를 강제**한다.
- 롤백 지점: Git 태그 `shipdb-pre-legacy-delete-20260725` (R2 스냅샷 계획은 폐기).

**위생 패치 — 2026-07-28 종료 확정**
| 항목 | 상태 |
|---|---|
| P1 초기 로드 중복 재렌더 | 완료 `7ab4c43` — 초기화 전체 재렌더 2회→1회, 요청 수·순서 불변 |
| R1 문서 정합 | 완료 `9455901` |
| P2 CSS 중복 병합 | **완료** `210ddb5` — 속성 충돌 0인 7건만, computed style 전 속성 0건 불일치 |
| R2 canonical 플래그 제거 | 완료 `2ec4dee` |
| 줄바꿈 정규화 | 완료 `de573c4` — `.gitattributes` 도입, index CRLF 28·혼합 10 → 0 |
| P4 테스트 고정 대기 | **완료** `6c3fa88` — 완료 신호가 확실한 admin 1건만 교체 |
| P3 main.js 모듈 분리 | **중단** — 실제 버그·기능 개발 필요 시에만 재개 |
| P2 잔여 CSS 중복 21건 | **중단** — `.hero` 계열, 속성 충돌 있어 병합 위험 |

**공지/갤러리 날짜 계약 (2026-08-02~03)**
- `b7507ab` 공지 날짜 저장 포맷을 `YYYY-MM-DD`로 고정 + 읽기 3곳 정렬을 포맷 비의존으로 +
  마이그레이션 `0012` + 린트 게이트(`biome --error-on-warnings`).
- `80e0ba3` 공지 빈 날짜가 저장되던 결함 수정(`limitText`가 `''`를 값으로 취급).
- `762fe73` 갤러리도 동일 패턴 수정. 갤러리는 date로 정렬하지 않아 포맷 정규화는 하지 않는다.
- **운영 D1**: `0012` 적용 완료, 중복 공지 3쌍 삭제, `ann-006` 날짜 복구. 상세는 운영 런북 3절.

**유예·미확인**
- D1 `ship_overrides` 레거시 컬럼 11개 — 읽지도 쓰지도 않는 정지 데이터. 백업·복구 리허설 확인 후
  별도 마이그레이션으로만 삭제한다.
- `0008`~`0011`의 운영 적용 여부 **미확인** — `SELECT id, applied_at FROM schema_migrations ORDER BY id;`
- Windows playwright 종료 간헐 지연은 **"관찰 대기"**. 재발·증거 수집 전까지 설정 변경 금지.

**다음**: VOLT AI 템플릿 베타(Workers AI 바인딩 없이, Discord 멤버 대상, 48시간 관찰).
...
- **②로더+비공개 플래그 완료(PM 재정렬 순서 1)**: `js/shipdb-canonical.js` — 내부 전환값 기본 OFF(URL·스토리지로 못 켬), ON은 테스트 훅(`__VOLT_SHIPDB_CANONICAL_TEST__`)만. OFF=fetch 0·store 비움·라이브 불변, ON=**core 8계층**(Erkul canonical 219·localization 219·operational 219·aliases 7·RSI official 30·RSI localization 30·role·taxonomy)을 해시 검증 후 함께 로드한다.
- **③RSI 공식 30척 메인 통합 완료**: 별도 `shipdb-rsi-catalog.js`·카탈로그 탭을 제거했다. ON 메인 목록은 **249척**(Erkul live 219 + RSI official 30)이며, 중복 에디션 별칭 7척만 제외한다. RSI 레코드는 상태 배지(concept 28 / flight-ready 2), 공식 설명·제조사·역할·크기·승무원·화물만 표시한다. `미구현 제외`은 concept 28만 숨기고 ATLS/ATLS GEO는 유지하며, RSI 30척은 무역플래너에서 제외한다.
...
- **⑥비교 하네스 + ON 메인 공개 집합 정합 완료(PM C)**: ON 메인 ShipDB는 공식 공개 집합 249척(Erkul 219 + RSI 30)을 렌더하고, 별칭 7척만 리다이렉트로 제외한다. 비교 하네스 `shipdb-compare-harness.spec.js`는 OFF 256·priceUsd / ON 249·priceUsd 제거·RSI 상태 28/2 / OFF∖ON=별칭 7을 고정한다.
