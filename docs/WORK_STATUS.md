...
- **다음(듀얼리드+플래그 승인, PM A 후속 순서)**: ①29척 RSI EN→KO 번역+해시(expanse는 "RSI 공식 설명 미제공" 상태만) → ②RSI 공식 카탈로그 탭/필터를 플래그 ON 경로에만 구현(OFF 라이브 미노출) → ③28컨셉+2ATLS 전부 플래너·비교·AI 제외 → ④canonical/localization/operational/rsi-official 로더+OFF 기본 플래그 → ⑤첫 원자 이관 `priceUsd`. ON 전환·삭제는 3.5.
