...
## 접근성 부채 — 상환 완료 (2026-07-06, `ebdf271`)

critical/serious allowlist는 전부 비웠고 래칫이 최강 상태다. 새 위반은 즉시 테스트 실패.

- **heading-order** — 섹션 제목 단계(h2→h3) 정돈. axe moderate(래칫 범위 밖), 낮은 우선순위로 유지.
...
- **cascade 중복 selector 129건** (155→129, `4290c7e`) — 완전 피복(dead) 블록 38건은 computed style
  ...
  속성 단위 병합이 필요하고, 스크린샷 회귀 도입 전에는 위험.
  진단: `node scripts/check-css-duplicates.mjs`. 회귀 가드: `css-regression.spec.js`의 cascade 가드 2종.
  파일 분리(`css/sections/*`)는 그 다음 단계.
...
## main.js 잔여 모듈화 (2054줄 — notices 분리 완료 `8af04fb`)

남은 렌더 책임(독립성 순): 일정/RSVP → 임원진/연혁 → FAQ/가이드/정책 → CMS fetch client 공통화.
...
1. (중간) main.js 일정/RSVP 렌더 분리 — notices 패턴 재적용.
2. (낮음·상시) CSS 중복 129건 — 스크린샷 회귀 도입 후 속성 단위 병합.
3. (낮음) heading-order 정돈.
4. (대기) 신규 함선 보류 8척 — 인게임 판매 시작이 트리거.
5. (선택) 함선 데이터 값 EN화.
