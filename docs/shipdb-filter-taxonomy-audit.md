...
## 커밋 C 완료 (2축 필터 UI + 카드 태그)

- **필터 UI**: 규모·플랫폼 축[전체·지상·소형·중형·대형·캐피탈] + 역할 축[14태그, 정제 0척 숨김] + 세부 역할 검색 콤보박스. 같은 축 OR · 축 간 AND · 세부 검색이 태그 결과를 추가로 좁힘 · 전체=해당 축만 초기화.
- **카드 태그**: 규모·플랫폼 1 + 역할 최대 2 + 세부 원문 역할 KO 라벨 1(`.ship-tag-size`/`.ship-tag-role`/`.ship-role-badge`).
- **i18n·a11y·반응형**: KO/EN 축·태그 라벨, `role=group`·`aria-pressed`·combobox/listbox, 데스크톱 줄바꿈·모바일 축 가로 스크롤.
- **검증**: `shipdb-filter-2axis.spec`(OR·AND·세부검색·초기화·카드태그·모바일 390) + 로더 8계층. 게이트 통과, 시각 기준 재생성.
