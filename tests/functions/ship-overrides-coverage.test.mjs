...
test('공개 ship-overrides GET: 표시 이름과 숨김 외 모든 source override 생략', async () => {
    // D1 행에 레거시 컬럼 값이 남아 있어도(정지 데이터) 공개 응답에는 실리지 않아야 한다.
    ...
    for (const f of CANONICAL_OMITTED) assert.ok(!(f in item), `${f} 생략돼야`);
