...
//  허용 차이: 메인 256→219 / RSI 카탈로그 30척 별도 / priceUsd·focus·tags 제거 / crew·cargo=Erkul.
//  그 외(공유 219척의 name·manufacturer·role·size·cargo 값·동작)는 OFF와 동일해야 한다.
...
test.describe('3.1 전후 비교 (허용 차이만, 그 외 실패)', () => {
    ...
    test('공유 219척: name·manufacturer·cargo 동일 / focus·tags·price 만 제거', async ({ browser }) => {
        const off = await loadState(await (await browser.newContext()).newPage(), false);
        const on = await loadState(await (await browser.newContext()).newPage(), true);
        ...
        for (const id of shared) {
            ...
            // 허용된 제거: focus 배지·태그·가격 스탯
            if (!a.focusBadge) unexpected.push(`${id}: OFF focus 배지 없음(기준선 위반)`);
            if (b.focusBadge) unexpected.push(`${id}: ON focus 배지 잔존`);
    ...
    test('컨트롤·비교: 정렬 price 옵션·태그 필터 제거, 비교 focus·price 행 제거·role 유지', async ({ page }) => {
        // ON
