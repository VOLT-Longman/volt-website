...
test.describe('함선 DB', () => {
    ...
    test('영어 함선DB: 헤더·필터칩·카드·모달 영어 표기', async ({ browser }) => {
        ...
        // 2축 태그 필터·세부 역할 검색이 영어: 축 라벨·세부 검색 placeholder·옵션 EN 원문(커밋 C)
        await expect(page.locator('#ship-tag-filters')).toContainText('Size & Platform');
        await expect(page.locator('#ship-tag-filters')).toContainText('Combat');
        await expect(page.locator('#ship-role-search')).toHaveAttribute('placeholder', /Search detailed roles/);
